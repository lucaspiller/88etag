(function() {
  var AiPlayer, Bullet, BulletsStorage, CommandCentre, CommandCentreInner, Controller, HealthBall, LocalPlayer, MassDriver, MassDriverFire, Movable, Player, PlayerShip, ShipBullet, ShipTrail, Starfield, Trail, TrailsStorage, Turret, TurretBase, TurretBullet, TurretBulletTrail, Universe,
  ...
  Controller = (function() {
    ...
    Controller.prototype.setupScene = function() {
      ...
      this.screen_range_x = Math.tan(this.camera.fov * Math.PI / 180) * this.camera.position.z * 2;
      this.screen_range_y = this.screen_range_x / this.camera.aspect;
      this.scene.add(new THREE.AmbientLight(0x999999));
      this.light = new THREE.PointLight(0xffffff);
      return this.scene.add(this.light);
    };

    Controller.prototype.camera_x_min = function() {
      return this.camera.position.x - this.screen_range_x / 2;
    };

    Controller.prototype.camera_x_max = function() {
      return this.camera.position.x + this.screen_range_x / 2;
    };

    Controller.prototype.camera_y_min = function() {
      return this.camera.position.y - this.screen_range_y / 2;
    };

    Controller.prototype.camera_y_max = function() {
      return this.camera.position.y + this.screen_range_y / 2;
    };
  ...
  Starfield = (function() {
    ...
    function Starfield(controller) {
      ...
      for (p = 0; 0 <= PARTICLE_COUNT ? p <= PARTICLE_COUNT : p >= PARTICLE_COUNT; 0 <= PARTICLE_COUNT ? p++ : p--) {
        ...
        pX = Math.random() * this.controller.screen_range_x - (this.controller.screen_range_x / 2);
        pY = Math.random() * this.controller.screen_range_y - (this.controller.screen_range_y / 2);
    ...
    Starfield.prototype.step = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        particle = _ref[_i];
        if (particle.position.x < this.controller.camera_x_min) {
          while (particle.position.x < this.controller.camera_x_min) {
            particle.position.x += this.controller.screen_range_x;
          }
        } else if (particle.position.x > this.controller.camera_x_max) {
          while (particle.position.x > this.controller.camera_x_max) {
            particle.position.x -= this.controller.screen_range_x;
          }
        }
        if (particle.position.y < this.controller.camera_y_min) {
          _results.push((function() {
            var _results2;
            _results2 = [];
            while (particle.position.y < this.controller.camera_y_min) {
              _results2.push(particle.position.y += this.controller.screen_range_y);
            }
            return _results2;
          }).call(this));
        } else if (particle.position.y > this.controller.camera_y_max) {
          _results.push((function() {
            var _results2;
            _results2 = [];
            while (particle.position.y > this.controller.camera_y_max) {
              _results2.push(particle.position.y -= this.controller.screen_range_y);
            }
            return _results2;
          }).call(this));
