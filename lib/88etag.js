(function() {
  ...
  BulletsStorage = (function() {
    ...
    BulletsStorage.prototype.newShipBullet = function(parent) {
      ...
      for (i = 1; i <= 2; i++) {
        ...
        _results.push(bullet.setup(parent.position, parent.rotation));
      }
      return _results;
    };
  ...
  ShipBullet = (function(_super) {
    ...
    ShipBullet.prototype.setup = function(position, rotation) {
      this.position.set(position.x, position.y, position.z - 10);
      this.velocity.x = Math.cos(rotation);
      this.velocity.y = Math.sin(rotation);
