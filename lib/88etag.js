define(function(require, exports, module){
// Generated by CoffeeScript 1.3.1
var __hasProp = {}.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor; child.__super__ = parent.prototype; return child; },
  __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };

$(document).ready(function() {
  var GameMenu, GameOverView, IndexView, PlayView, Router, router;
  GameMenu = require('game_menu');
  require('templates');
  if (!Detector.webgl) {
    Detector.addGetWebGLMessage();
    return;
  }
  IndexView = (function(_super) {

    __extends(IndexView, _super);

    IndexView.name = 'IndexView';

    function IndexView() {
      return IndexView.__super__.constructor.apply(this, arguments);
    }

    IndexView.prototype.render = function() {
      var playLink;
      playLink = $('<a/>').attr('href', '#play').text('Play');
      this.$el.append(playLink);
      return this;
    };

    IndexView.prototype.dispose = function() {
      return true;
    };

    return IndexView;

  })(Backbone.View);
  GameOverView = (function(_super) {

    __extends(GameOverView, _super);

    GameOverView.name = 'GameOverView';

    function GameOverView() {
      return GameOverView.__super__.constructor.apply(this, arguments);
    }

    GameOverView.prototype.className = 'game_over';

    GameOverView.prototype.template = JST['game_over'];

    GameOverView.prototype.render = function() {
      this.$el.append(this.template);
      return this;
    };

    GameOverView.prototype.dispose = function() {
      return this.$el.remove();
    };

    return GameOverView;

  })(Backbone.View);
  PlayView = (function(_super) {

    __extends(PlayView, _super);

    PlayView.name = 'PlayView';

    function PlayView() {
      this.gameOver = __bind(this.gameOver, this);

      this.keyUp = __bind(this.keyUp, this);

      this.keyDown = __bind(this.keyDown, this);
      return PlayView.__super__.constructor.apply(this, arguments);
    }

    PlayView.prototype.reset = function() {
      this.dispose();
      this.$el.html('');
      return this.render();
    };

    PlayView.prototype.render = function() {
      var Engine;
      Engine = require('engine/engine');
      this.engine = new Engine({
        container: this.$el.get(0),
        aiPlayers: 1,
        onKeyDown: this.keyDown,
        onKeyUp: this.keyUp,
        onGameOver: this.gameOver
      });
      return this;
    };

    PlayView.prototype.dispose = function() {
      this.engine.dispose();
      this.engine = false;
      if (this.menu) {
        this.menu.dispose();
      }
      this.menu = false;
      if (this.gameOverView) {
        this.gameOverView.dispose();
      }
      return this.gameOverView = false;
    };

    PlayView.prototype.keyDown = function(key) {
      if (key === 27) {
        router.navigate('', true);
        return;
      }
      if (this.menu) {
        switch (key) {
          case 37:
            this.menu = this.menu.left(this.engine);
            return false;
          case 39:
            this.menu = this.menu.right(this.engine);
            return false;
          case 38:
            this.menu = this.menu.up(this.engine);
            return false;
          case 40:
            this.menu = this.menu.down(this.engine);
            return false;
        }
      }
      if (this.gameOverView) {
        if (key === 32) {
          this.reset();
        }
        return false;
      } else {
        if (key === 65) {
          this.menu = new GameMenu;
          this.$el.append(this.menu.render().el);
        }
      }
      return true;
    };

    PlayView.prototype.keyUp = function(key) {
      if (key === 65) {
        if (this.menu) {
          this.menu = this.menu.dispose();
        }
      }
      return true;
    };

    PlayView.prototype.gameOver = function() {
      this.gameOverView = new GameOverView;
      return this.$el.append(this.gameOverView.render().el);
    };

    return PlayView;

  })(Backbone.View);
  Router = (function(_super) {

    __extends(Router, _super);

    Router.name = 'Router';

    function Router() {
      return Router.__super__.constructor.apply(this, arguments);
    }

    Router.prototype.routes = {
      '': 'showIndex',
      'play': 'showPlay'
    };

    Router.prototype.showIndex = function() {
      var indexView;
      indexView = new IndexView;
      return this.changeView(indexView);
    };

    Router.prototype.showPlay = function() {
      var playView;
      playView = new PlayView;
      return this.changeView(playView);
    };

    Router.prototype.changeView = function(view) {
      if (this.currentView) {
        this.currentView.dispose();
      }
      $('body').html(view.render().el);
      return this.currentView = view;
    };

    return Router;

  })(Backbone.Router);
  router = new Router;
  return Backbone.history.start();
});

});
