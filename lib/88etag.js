(function() {
  var AiPlayer, Bullet, BulletsStorage, CommandCentre, CommandCentreInner, Controller, HealthBall, Indicator, LocalPlayer, MassDriver, MassDriverFire, Movable, Player, PlayerShip, ShipBullet, ShipTrail, Starfield, Trail, TrailsStorage, Turret, TurretBase, TurretBullet, TurretBulletTrail, Universe,
  ...
  Indicator = (function(_super) {
    ...
    Indicator.prototype.solid = false;

    Indicator.prototype.buildMesh = function() {
    ...
    function Indicator(options) {
      ...
      _ref = this.controller.screen_range(650), this.screen_range_x = _ref[0], this.screen_range_y = _ref[1];
    }

    Indicator.prototype.step = function() {
      Indicator.__super__.step.apply(this, arguments);
      if (this.universe.player === this.parent) {
        this.mesh.material.color.setRGB(89 / 255, 163 / 255, 89 / 255);
      }
      ...
      this.xOnScreen = false;
      this.yOnScreen = false;
      if (this.parent.commandCentre.position.x < this.camera_x_min) {
        this.position.x = this.camera_x_min;
      } else if (this.parent.commandCentre.position.x > this.camera_x_max) {
        this.position.x = this.camera_x_max;
      } else {
        this.xOnScreen = true;
      }
      if (this.parent.commandCentre.position.y < this.camera_y_min) {
        this.position.y = this.camera_y_min;
      } else if (this.parent.commandCentre.position.y > this.camera_y_max) {
        this.position.y = this.camera_y_max;
      } else {
        this.yOnScreen = true;
      }
      if (this.xOnScreen && this.yOnScreen) {
        return this.position.z = HIDDEN_Z;
      } else {
        return this.position.z = 575;
      }
    };
