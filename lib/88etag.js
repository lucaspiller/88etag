(function() {
  var CommandCentre, Controller, LocalPlayer, Movable, Player, ShipTrail, Starfield, TrailsContainer, Universe,
  ...
  Universe = (function() {
    ...
    function Universe(controller) {
      ...
      this.trails = new TrailsContainer({
    ...
    Universe.prototype.checkCollisions = function() {
      var m1, m2, _i, _len, _ref, _results;
      _ref = this.masses;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        m1 = _ref[_i];
  ...
  Movable = (function() {
    ...
    Movable.prototype.step = function() {
      ...
      if (Math.abs(this.rotationalVelocity) > 0) {
        ...
        return this.rotation = (this.rotation + this.rotationalVelocity) % (Math.PI * 2);
      }
    };
  ...
  ShipTrail = (function(_super) {
    ...
    ShipTrail.prototype.setup = function(position, velocity) {
      this.position.set(position.x, position.y, position.z - 10);
      this.velocity.set(Math.random() - 0.5, Math.random() - 0.5, 0);
    ...
    ShipTrail.prototype.step = function() {
      if (this.alive) {
        if (this.lifetime > 0) {
          this.position.addSelf(this.velocity);
          this.mesh.material.color.r -= 1 / 30;
          this.mesh.material.color.g -= 1 / 30;
          this.mesh.material.color.b -= 1 / 30;
  ...
  TrailsContainer = (function() {

    function TrailsContainer(options) {
      this.universe = options.universe;
      this.controller = options.controller;
      this.pool = [];
    }

    TrailsContainer.prototype.addToPool = function(trail) {
      return this.pool.push(trail);
    };

    TrailsContainer.prototype.newShipTrail = function(parent) {
      var trail;
      trail = this.pool.pop();
      if (!trail) {
        trail = new ShipTrail({
      ...
      return trail.setup(parent.position, parent.velocity);
    };
  ...
  Player = (function(_super) {
    ...
    Player.prototype.forward = function() {
      ...
      return this.universe.trails.newShipTrail(this);
    };
    ...
    Player.prototype.step = function() {
      ...
      this.mesh.rotateAboutObjectAxis(THREE.AxisX, Math.PI / 128);
